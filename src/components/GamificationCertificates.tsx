...
import {
  ...
  Flame, FileText, FolderOpen, Lock, ChevronRight, Sprout,
} from 'lucide-react';
...
const CERTIFICATE_LEVELS: CertificateLevel[] = [
  {
    ...
    level: 0,
    title: 'First Step',
    subtitle: 'Welcome to Flowist',
    icon: Sprout,
    requirements: { tasksCompleted: 0, streakDays: 0, notesCreated: 0, foldersUsed: 0 },
    certificateText: 'This certifies that the holder has taken their very first step on the productivity journey with Flowist. By creating their first task, note, and sketch, they have demonstrated the initiative to organize their life and unleash their creative potential. Every great journey begins with a single step.',
    linkedInDescription: '🏅 Just earned my Flowist "First Step" Certificate!\n\nI created my first task, note, and sketch on Flowist — and the journey has officially begun! ✅📝🎨\n\nSmall beginnings lead to extraordinary results. Here\'s to building better habits, one step at a time.\n\n#Productivity #FirstStep #Flowist #PersonalGrowth #NewBeginnings',
    colors: {
      bg: 'linear-gradient(160deg, hsl(160, 30%, 10%), hsl(170, 35%, 14%))',
      accent: 'hsl(160, 70%, 50%)',
      text: 'hsl(160, 55%, 72%)',
      border: 'hsl(160, 40%, 25%)',
      glow: 'hsl(160, 70%, 50%, 0.15)',
    },
  },
  {
    id: 'beginner',
...
const isUnlocked = (progress: UserProgress, cert: CertificateLevel, firstStepEarned?: boolean): boolean => {
  if (cert.id === 'first_step') return !!firstStepEarned;
  const r = cert.requirements;
  return (
...
export const hasNewCertificates = async (longestStreak: number): Promise<boolean> => {
  try {
    ...
    return CERTIFICATE_LEVELS.some(
      cert => isUnlocked(progress, cert, !!firstStepData) && !seenCerts.includes(cert.id)
    );
  } catch {
    return false;
  }
...
export const GamificationCertificates = ({ isOpen, onClose, streakData }: CertificatesProps) => {
  ...
  const [firstStepEarned, setFirstStepEarned] = useState(false);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const hasFirstStep = !!firstStepData;
        setFirstStepEarned(hasFirstStep);
        ...
        const newlyUnlocked = CERTIFICATE_LEVELS.filter(
          cert => isUnlocked(userProgress, cert, hasFirstStep) && !seenCerts.includes(cert.id)
        );
      ...
      >
        ...
        {isLoading || !progress ? (
          ...
          <CertificateDetail
            cert={selectedCert}
            progress={progress}
            unlocked={isUnlocked(progress, selectedCert, firstStepEarned)}
            cardRef={cardRef}
          ...
          <div className="px-4 py-6 space-y-3 pb-32">
            ...
            {CERTIFICATE_LEVELS.map((cert, i) => {
              const unlocked = isUnlocked(progress, cert, firstStepEarned);
