...
export const TaskInputSheet = ({ isOpen, onClose, onAddTask, folders, selectedFolderId, onCreateFolder, sections = [], selectedSectionId, defaultDate, preventBackdropClose = false }: TaskInputSheetProps) => {
  ...
  // Keyboard height detection for platforms that still need manual offset.
  // Android native already repositions fixed bottom elements with the visual viewport,
  // so applying keyboardHeight there causes a double lift and leaves a large gap.
  const keyboardHeight = useKeyboardHeight();
  const isAndroidNative = typeof document !== 'undefined' && document.body.classList.contains('android-app');
  const isKeyboardOpen = keyboardHeight > 0;
  const sheetBottom = !isAndroidNative && isKeyboardOpen ? `${keyboardHeight}px` : '0px';
  const sheetPaddingBottom = isKeyboardOpen ? '0px' : 'var(--safe-bottom, 8px)';
